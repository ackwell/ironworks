...
impl Recipe {
    pub fn populate(row: &Row, offset: usize) -> Result<Self, PopulateError> {
        Result::Ok(Self {
            ...
            r#required_craftsmanship: row.field(32usize + offset)?.into_u16()?,
            r#required_control: row.field(33usize + offset)?.into_u16()?,
            r#quick_synth_craftsmanship: row.field(34usize + offset)?.into_u16()?,
            r#quick_synth_control: row.field(35usize + offset)?.into_u16()?,
            r#secret_recipe_book: row.field(36usize + offset)?.into_u16()?,
            r#quest: row.field(37usize + offset)?.into_u32()?,
            r#can_quick_synth: row.field(38usize + offset)?.into_bool()?,
            r#can_hq: row.field(39usize + offset)?.into_bool()?,
            r#exp_rewarded: row.field(40usize + offset)?.into_bool()?,
            r#status_required: row.field(41usize + offset)?.into_i32()?,
            r#item_required: row.field(42usize + offset)?.into_i32()?,
            r#is_specialization_required: row.field(43usize + offset)?.into_bool()?,
            r#is_expert: row.field(44usize + offset)?.into_bool()?,
            r#patch_number: row.field(45usize + offset)?.into_u8()?,
        })
    }
}
