...
impl Companion {
    pub fn populate(row: &Row, offset: usize) -> Result<Self, PopulateError> {
        Result::Ok(Self {
            ...
            r#priority: row.field(17usize + offset)?.into_u8()?,
            ...
            r#enemy: row.field(23usize + offset)?.into_bool()?,
            r#battle: row.field(24usize + offset)?.into_bool()?,
            r#roulette: row.field(25usize + offset)?.into_u16()?,
            r#unknown26: row.field(26usize + offset)?.into_bool()?,
            r#unknown27: row.field(27usize + offset)?.into_bool()?,
            r#icon: row.field(28usize + offset)?.into_u16()?,
            r#order: row.field(29usize + offset)?.into_u16()?,
            r#idle_animation: row.field(30usize + offset)?.into_bool()?,
            r#unknown31: row.field(31usize + offset)?.into_u8()?,
            r#cost: row.field(32usize + offset)?.into_u8()?,
            r#hp: row.field(33usize + offset)?.into_u16()?,
            r#unknown34: row.field(34usize + offset)?.into_u8()?,
            r#skill_angle: row.field(35usize + offset)?.into_u16()?,
            r#skill_cost: row.field(36usize + offset)?.into_u8()?,
            r#unknown37: row.field(37usize + offset)?.into_u8()?,
            r#unknown38: row.field(38usize + offset)?.into_u16()?,
            r#minion_race: row.field(39usize + offset)?.into_u8()?,
        })
    }
}
