...
impl RecipeLevelTable {
    pub fn populate(row: &Row, offset: usize) -> Result<Self, PopulateError> {
        Result::Ok(Self {
            ...
            r#difficulty: row.field(3usize + offset)?.into_u16()?,
            r#quality: row.field(4usize + offset)?.into_u32()?,
            r#progress_divider: row.field(5usize + offset)?.into_u8()?,
            r#quality_divider: row.field(6usize + offset)?.into_u8()?,
            r#progress_modifier: row.field(7usize + offset)?.into_u8()?,
            r#quality_modifier: row.field(8usize + offset)?.into_u8()?,
            r#durability: row.field(9usize + offset)?.into_u16()?,
            r#conditions_flag: row.field(10usize + offset)?.into_u16()?,
        })
    }
}
